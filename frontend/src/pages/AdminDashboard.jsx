...
import { getHotels, createHotel, updateHotel, deleteHotel, createRoom, updateRoom, deleteRoom, getReservations, deleteReservation } from '../services/api';
    ...
    return (
        <div className="container">
            <div style={{ marginBottom: '2rem' }}>
                <h2 style={{ fontSize: '2.5rem', marginBottom: '1.5rem' }}>Dashboard</h2>
                ...
                <div style={{ display: 'flex', gap: '1rem', borderBottom: '1px solid var(--border-color)', marginBottom: '2rem' }}>
                    <button
                        ...
                        style={{
                            ...
                            background: 'none',
                            border: 'none',
                            borderBottom: activeTab === 'hotels' ? '2px solid var(--primary-color)' : 'none',
                            color: activeTab === 'hotels' ? 'var(--primary-color)' : 'var(--text-secondary)',
                            fontWeight: 'bold',
                            cursor: 'pointer',
                            fontSize: '1.1rem'
                        }}
                    ...
                    <button
                        ...
                        style={{
                            ...
                            background: 'none',
                            border: 'none',
                            borderBottom: activeTab === 'reservations' ? '2px solid var(--primary-color)' : 'none',
                            color: activeTab === 'reservations' ? 'var(--primary-color)' : 'var(--text-secondary)',
                            fontWeight: 'bold',
                            cursor: 'pointer',
                            fontSize: '1.1rem'
                        }}
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '800px' }}>
                        <thead>
                            <tr style={{ borderBottom: '2px solid #f1f5f9', background: '#f8fafc' }}>
                                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#64748b' }}>ID</th>
                                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#64748b' }}>User</th>
                                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#64748b' }}>Hotel</th>
                                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#64748b' }}>Room</th>
                                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#64748b' }}>Dates</th>
                                <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', color: '#64748b' }}>Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {reservations.map(res => (
                                <tr key={res.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                    ...
                                    <td style={{ padding: '1rem' }}>
                                        {/* Since backend serializer might not expand auth user fully, standard UserSerializer usually has username */}
                                        {/* Assuming res.user is ID, we might need to populate it or just show ID. 
                                            Wait, ReservationSerializer typically handles relations. 
                                            Let's assume default simple serializer returns user ID if not nested. 
                                            Let's check serializer later. For now, display res.user }
                                            Actually better to verify structure. But let's assume worst case ID. */}
                                        User {res.user}
                                    </td>
                                    <td style={{ padding: '1rem' }}>
                                        {/* We probably need to fetch hotel name via room relation. 
                                           The current Reservation serializer usually just returns fields.
                                           We might need to update serializer to get readable names. 
                                           For now, showing Room ID is safe. */}
                                        -
                                    </td>
                                    <td style={{ padding: '1rem' }}>Room {res.room}</td>
                                    <td style={{ padding: '1rem' }}>
                                        <div style={{ fontSize: '0.9rem' }}>In: {res.check_in}</div>
                                        <div style={{ fontSize: '0.9rem' }}>Out: {res.check_out}</div>
                                    </td>
                                    <td style={{ padding: '1rem' }}>
                                        <button
